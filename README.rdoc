...
a matter of adding a line of code to your controller and creating a view to display your data.
...
To install, configure and initialize will_filter engine, run the following commands: 
  ...
  $ script/generate will_filter_migration
  $ rake db:migrate 
...
Here is an example of how objects can be filtered in your controller:

  class ProfileController < ApplicationController
  
    def index
      @profiles = Profile.filter(:params => params)    
    end
...
That's it. Now all you have to do is create a view for this action and add the following tag where you want your filter to be rendered:

  <%= will_filter(@profiles) %>
